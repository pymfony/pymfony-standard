...
def pkg_to_mapping(name):
    ...
    for root, dirs, files in os.walk(toplevel, followlinks=True):
        if ( '__init__.py' not in files
            and not root.endswith('__pycache__')
            and not root.endswith('Resources')
            ):
            pkg = pkgname(name, toplevel, os.path.join(root, '__init__.py'))
            name2src[pkg] = "";
        for pyfile in files:
            if (fnmatch.fnmatch(pyfile, '*.py')
                or fnmatch.fnmatch(pyfile, '*.json') # Resources
                or fnmatch.fnmatch(pyfile, '*.ini')
                ):
